                    ...
                    uint8_t dataWidth)
{
    ...
    DBG_APP_INFO("I2C: Write [Slave=0x%x, RegAddr=0x%x, Data=0x%x]\r\n", slaveAddress, registerAddress, data);
...
cy_en_scb_i2c_status_t Cy_APP_GetFPGAVersion(cy_stc_usb_app_ctxt_t *pAppCtxt)
{
    ...
    status = Cy_I2C_Read(FPGASLAVE_ADDR, DEVICE_FLAG_STATUS_ADDRESS(0),
...
void ConfigureSCB0Clock(uint8_t scbIndex)
{
    ...
    DBG_APP_INFO("I2C: SCB Index %d HFCLK %d\r\n", scbIndex, hfClkFreq);

    /* Identify the divider value required to obtain a 3 MHz clock by dividing the PERI clock frequency */
    divider = (hfClkFreq / 3000000UL) - 1;
    Cy_SysClk_PeriphSetDivider(CY_SYSCLK_DIV_16_BIT, 3, divider);
...
void Cy_USB_I2CInit (void)
{
    ...
    DBG_APP_INFO("I2C: Data Clock %d\r\n", dataClock);
    DBG_APP_INFO("I2C: Data Rate %d\r\n", dataRate);
